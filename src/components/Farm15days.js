...
import {INFURA, WALLET_LPPIS, WALLET_PIS, CONTRACT_LPPIS, DECIMAL,TOTAL_APPROVE, CONTRACT_OWNER, TIME_15days} from '../constants'
export default class Farm15days extends Component{
    constructor(props) {
        super(props)
        this.state = { 
           amount:0,
    ...
    stake(){
        ...
        this.props.contractPiswap.methods.stake(UUI, balance, 2, this.props.addressRef, sponsor).send({ from: this.props.account }).on('confirmation', (confNumber, receipt) => {
            if(confirm==0){
       ...
       if(this.props.stateconnect){
            if(this.state.enableuse){
                listhistory = this.state.listcurrentpage.map((list) => {
                    ...
                    const timeregis = Number(list.timewithdraw) - TIME_15days;
